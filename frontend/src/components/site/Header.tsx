...
export function Header({ menuAberto = false, onMenuEnter }: HeaderProps) {
            ...
            <button
              ...
              className={`group grid h-10 w-10 place-items-center transition-colors ${
                menuAberto
                  ...
                  : "bg-transparent text-foreground hover:bg-brand-red hover:text-white"
              }`}
              aria-label="Abrir menu"
            >
              <div className="relative h-[22px] w-[22px]">
                <span
                  className={`absolute h-[2px] rounded-full bg-current transition-all duration-300 ease-in-out ${
                    menuAberto
                      ? "left-[2px] top-[16px] w-[10px] origin-right rotate-45"
                      : "left-0 top-[3px] w-[22px] rotate-0"
                  }`}
                />

                <span
                  className={`absolute rounded-full bg-current transition-all duration-300 ease-in-out ${
                    menuAberto
                      ? "left-[10px] top-[3px] h-[15px] w-[2px]"
                      : "left-0 top-[10px] h-[2px] w-[22px]"
                  }`}
                />
                ...
                <span
                  className={`absolute h-[2px] rounded-full bg-current transition-all duration-300 ease-in-out ${
                    menuAberto
                      ? "left-[10px] top-[16px] w-[10px] origin-left -rotate-45"
                      : "left-0 top-[17px] w-[22px] rotate-0"
                  }`}
                />
              </div>
            </button>
